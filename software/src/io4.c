...
void io4_init(void) {
  ...
  for(uint8_t i = 0; i < NUMBER_OF_CHANNELS; i++) {
    ...
    XMC_GPIO_Init(io4.channels[i].port_base,
                  io4.channels[i].pin,
  ...
  ringbuffer_init(&io4.all_input_value_cb.cb_rb,
                  ALL_INPUT_VALUE_CB_BUFFER_SIZE,
                  &io4.all_input_value_cb.cb_buffer[0]);

  ringbuffer_init(&io4.input_value_cb_rb,
...
void io4_tick(void) {
  ...
  for(uint8_t i = 0; i < NUMBER_OF_CHANNELS; i++) {
    if(io4.channels[i].direction == IO4_V2_DIRECTION_IN) {
      ...
      // Manage edge count
      if(system_timer_is_time_elapsed_ms(io4.channels[i].edge_count.debounce_start,
                                         io4.channels[i].edge_count.debounce)) {
